...
class Orchestrator:
    ...
    def _extract_topic(self, query: str) -> str:
        ...
        topic_keywords = [
            ...
            ("打车", ["打车", "滴滴", "高德", "专车", "快车", "网约车", "个车"]),
            ("旅游", ["旅游", "旅行", "景点", "酒店", "机票", "去哪玩", "出去玩"]),
            ("美食", ["美食", "餐厅", "餐馆", "菜", "吃", "推荐菜", "小吃", "甜品"]),
            # 通用话题放最后（避免过度匹配）
            ("购物", ["购物", "价格", "便宜", "对比", "买东西"]),
            ("选车", ["买车", "选车", "新能源车", "suv"]),  # "选车" 放后面，避免"车"字过度匹配
        ]
        ...
        if "买" in query_lower:
            ...
            for topic, keywords in topic_keywords:
                if any(kw in query_lower for kw in keywords):
                    return topic
            # 否则尝试提取"买"后面的物品（跳过数量词）
            match = re.search(r'买 [辆个件台瓶份点]?[\s]*([A-Za-z\u4e00-\u9fa5]+)', query_lower)
            if match:
                item = match.group(1)
                # 排除通用词
                exclude = ["什么", "一些", "一个", "东西", "物品"]
                if item not in exclude:
                    return item

        # 第四轮：提取问题中的关键词作为主题（排除通用词）
        # 排除容易误匹配的通用词
        exclude_words = ["一些", "一个", "一起", "一定", "分析", "建议", "帮助", "比较", "看看", "我想买"]
