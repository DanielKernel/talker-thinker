# Talker-Thinker 评测系统

## 概述

Talker-Thinker 评测系统是一个全面的自动化评测框架，用于评估双 Agent 系统（Talker 和 Thinker）的性能表现。

## 评测架构

### 分层评测设计

```
L1 单元测试 → 组件级别 (Agent、Skills、Context)
L2 集成测试 → Agent 协同、Handoff 机制
L3 端到端评测 → 完整用户场景
L4 质量评估 → 答案质量、相关性、准确性
```

### 核心评测维度

| 维度 | 指标 | 目标 |
|------|------|------|
| 响应速度 | TTFT, TPS | Talker <500ms |
| 意图识别准确率 | Classification Accuracy | >90% |
| Handoff 正确率 | Correct Routing | >95% |
| 任务完成率 | Task Success Rate | >85% |
| 答案质量 | Quality Score (0-100) | >80 |

## 目录结构

```
evals/
├── __init__.py              # 模块入口
├── __main__.py              # CLI 入口
├── harness.py               # 评测执行引擎
├── assertions.py            # 断言检查器库
├── cases/                   # 评测用例集
│   ├── __init__.py
│   ├── simple.py            # 简单任务 (S001-S006)
│   ├── medium.py            # 中等任务 (M001-M006)
│   ├── complex.py           # 复杂任务 (C001-C006)
│   └── edge.py              # 边界/异常 (E001-E008)
├── metrics/                 # 评测指标
│   ├── __init__.py
│   ├── latency.py           # 时延指标
│   ├── accuracy.py          # 准确率指标
│   ├── quality.py           # 质量评分
│   └── collector.py         # 指标收集器
├── reporters/               # 报告生成
│   ├── __init__.py
│   ├── console.py           # 控制台输出
│   ├── json_reporter.py     # JSON 导出
│   └── html.py              # HTML 报告
├── fixtures/                # 测试固件
│   ├── __init__.py
│   ├── mock_skills.py       # Mock 技能
│   └── test_data.py         # 测试数据
└── results/                 # 评测结果 (生成)
    ├── eval_result.json     # JSON 结果
    └── eval_report.html     # HTML 报告
```

## 快速开始

### 运行评测

```bash
# 激活虚拟环境
source .venv/bin/activate

# 运行全部评测
python -m evals run

# 运行评测并生成报告
python -m evals run -v --output result.json --html

# 运行特定类别的评测
python -m evals run --category simple
python -m evals run --category complex
```

### 列出评测用例

```bash
# 列出所有用例
python -m evals list

# 列出特定类别用例
python -m evals list --category complex
```

### 生成报告

```bash
# 从 JSON 生成 HTML 报告
python -m evals report --input result.json --format html

# 在控制台显示报告
python -m evals report --input result.json --format console
```

## CLI 命令详解

### `run` - 运行评测

```bash
python -m evals run [选项]

选项:
  --category, -c     运行特定类别 (simple/medium/complex/edge)
  --output, -o       输出 JSON 文件路径
  --html, -H         生成 HTML 报告
  --verbose, -v      显示详细输出
  --real-llm         使用真实 LLM (默认使用 Mock)
  --timeout          用例超时时间 (秒)
  --latency          Mock 响应延迟 (ms)
```

### `list` - 列出用例

```bash
python -m evals list [选项]

选项:
  --category, -c     列出特定类别的用例
```

### `report` - 生成报告

```bash
python -m evals report --input <文件> [选项]

选项:
  --input, -i        输入 JSON 文件 (必需)
  --output, -o       输出文件路径
  --format, -f       报告格式 (json/html/console)
```

## 评测用例设计

### 用例结构

每个评测用例 (`EvalCase`) 包含以下字段：

```python
@dataclass
class EvalCase:
    case_id: str                        # 唯一标识 (如 S001, M001)
    name: str                           # 用例名称
    description: str                    # 描述
    category: EvalCategory              # 类别
    user_input: str                     # 用户输入
    expected_complexity: TaskComplexity # 期望复杂度
    expected_agent: AgentRole           # 期望处理 Agent
    assertions: List[Assertion]         # 断言列表
    golden_output: Optional[str]        # 期望输出 (可选)
    priority: Priority                  # 优先级
    tags: List[str]                     # 标签
    context_messages: List[Dict]        # 前置上下文
```

### 断言设计

```python
@dataclass
class Assertion:
    name: str                        # 断言名称
    checker: Callable                # 检查函数
    weight: float                    # 权重
    failure_reason_template: str     # 失败原因模板
    description: str                 # 描述
```

### 用例分类

#### 简单任务 (Simple, S001-S006)
- 问候响应
- 简单计算
- 时间查询
- 记忆召回
- 自我介绍
- 感谢回应

#### 中等任务 (Medium, M001-M006)
- 天气查询
- 单位转换
- 电影推荐
- 翻译
- 定义解释
- 简单比较

#### 复杂任务 (Complex, C001-C006)
- 深度分析
- 产品对比
- 旅行规划
- 问题排查
- 系统设计
- 学术写作

#### 边界/异常 (Edge, E001-E008)
- 空输入
- 超长输入
- 多语言混合
- 模糊查询
- 特殊字符
- 重复问题
- 敏感话题
- 指令注入尝试

## 评测报告

### 控制台报告

控制台报告包含：
- 基本信息（评测 ID、时间、耗时）
- 总体概览（通过率、平均得分、响应时间）
- 分类统计（按难度分组）
- 失败原因分布
- 详细结果（每个用例）
- 目标达成情况
- 改进建议

### HTML 报告

HTML 报告提供：
- 可视化仪表板
- 彩色状态指示器
- 交互式表格
- 响应式设计

### JSON 导出

JSON 格式便于：
- 自动化处理
- 历史对比
- 集成到 CI/CD

## 失败原因分类

```python
class FailureReason(Enum):
    WRONG_AGENT = "wrong_agent"         # 路由错误
    TIMEOUT = "timeout"                 # 超时
    WRONG_OUTPUT = "wrong_output"       # 输出错误
    SKILL_FAILED = "skill_failed"       # 技能调用失败
    ASSERTION_FAILED = "assertion_failed" # 断言失败
    EXCEPTION = "exception"             # 异常
    HANDOFF_FAILED = "handoff_failed"   # Handoff 失败
    CONTEXT_LOST = "context_lost"       # 上下文丢失
```

## 扩展评测系统

### 添加新用例

```python
# 在 evals/cases/ 下创建或用例文件
from context.types import AgentRole, TaskComplexity
from ..core.types import Assertion, EvalCase, EvalCategory, Priority

NEW_CASES = [
    EvalCase(
        case_id="X001",
        name="new_feature_test",
        description="测试新功能",
        category=EvalCategory.MEDIUM,
        user_input="测试输入",
        expected_complexity=TaskComplexity.MEDIUM,
        expected_agent=AgentRole.TALKER,
        assertions=[
            Assertion(
                name="response_time_check",
                checker=lambda response_time_ms, **_: response_time_ms < 500,
                weight=1.0,
                failure_reason_template="响应时间超过 500ms",
                description="响应时间应小于 500ms",
            ),
        ],
        priority=Priority.HIGH,
    ),
]

def get_cases() -> list:
    return NEW_CASES
```

### 添加新断言

```python
# 在 evals/assertions.py 中添加新检查函数
def check_new_feature(actual_output: str, **kwargs) -> bool:
    """检查新功能"""
    return "expected" in actual_output.lower()
```

## 最佳实践

1. **运行前准备**: 确保配置好 Mock LLM 进行测试
2. **定期评测**: 每次重大改动后运行完整评测
3. **分析失败**: 查看失败原因分布，优先修复高频问题
4. **历史对比**: 保存 JSON 结果用于趋势分析
5. **CI/CD 集成**: 在 GitHub Actions 中自动运行评测

## 示例输出

```
================================================================================
                              Talker-Thinker 评测报告
================================================================================

📊 基本信息
------------------------------------------------------------
  评测 ID:        f5a2ccdc...
  评测时间：2026-02-22 13:18:18
  耗时：15.03 秒

📈 总体概览
------------------------------------------------------------
  总用例数：26
  通过用例：18 (69.2%)
  失败用例：8 (30.8%)
  平均得分：88.1/100
  平均响应时间：576.9ms

📋 分类统计
------------------------------------------------------------
  简单任务：100.0% 通过率，150.0ms 平均响应
  中等任务：83.3% 通过率，150.0ms 平均响应
  复杂任务：0.0% 通过率，1691.7ms 平均响应
  边界/异常：87.5% 通过率，381.2ms 平均响应

🎯 目标达成情况
------------------------------------------------------------
  ✅ 响应速度：实际 576.9ms (目标 <500ms)
  ❌ 通过率：实际 69.2% (目标 >85%)
  ✅ 平均得分：实际 88.1 (目标 >80)
```

## 故障排除

**问题**: 断言检查报错 `'<=' not supported between instances of 'AgentRole' and 'int'`

**解决**: 确保断言的 lambda 函数使用正确的参数名（`response_time_ms` 而非`actual`）

**问题**: HTML 报告无法生成

**解决**: 检查 `evals/results/` 目录是否存在且可写

**问题**: 复杂任务全部失败

**解决**: 检查 Mock Thinker 的输出长度是否满足断言要求
