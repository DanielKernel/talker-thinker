...
## 2026-02-21 会话 C：高德 vs 滴滴对比任务回放

### 原始片段（节选）
```
[16:29:54.173] 你：帮我对比下高德和滴滴，找几个便宜的选项

[16:30:11.243] Talker: 好的，这个问题需要深度思考，已转交给 Thinker 处理

[16:30:13.260] Talker: 正在同步上下文并规划步骤，请稍候...
[16:30:18.304] Talker: 正在分析关键信息...
[16:30:23.349] Talker: 正在核对必要条件...
[16:30:26.281] Talker: 预分析耗时较长，先进入详细处理...
[16:30:26.281] Thinker: 开始处理...
[16:30:26.281] Talker: 仍在准备分析，请稍候...
[16:30:26.331] Thinker: [思考] 正在分析任务...

[16:31:02.421] Talker: 仍在分析阶段（51s）
...
[16:31:50.621] Talker: 仍在执行阶段（99s）
[16:32:06.706] Talker: 仍在执行阶段（115s）
[16:32:22.756] Talker: 仍在执行阶段（132s）
[16:32:38.825] Talker: 仍在执行阶段（148s）  ✓ 完成 (87025ms)
[步骤 2] 搜索价格对比信息...

[16:32:54.886] Talker: 仍在执行阶段（164s）
[16:33:10.947] Talker: 仍在执行阶段（180s）
[16:33:27.001] Talker: 仍在执行阶段（196s）
[16:33:43.069] Talker: 仍在执行阶段（212s）
[16:33:59.132] Talker: 仍在执行阶段（228s）
[16:34:15.228] Talker: 仍在执行阶段（244s）
```

### 问题判定
1. **播报信息浪费**：Thinker 已经输出了详细进度（如"[步骤 1] 搜索平台基础信息..."、"[规划] 共 4 个步骤"），但 Talker 播报仍然是机械的"仍在执行阶段（83s）"，没有利用 Thinker 的真实进度信息。
2. **播报内容单一**：长时间（164s~244s）重复"仍在执行阶段"，用户无法感知具体进展。
3. **Thinker 输出直显**：Thinker 的阶段标记（如"[步骤 1] xxx"）直接显示给用户，Talker 没有重新组织语言。

### 已落地修复
1. **heartbeat 播报优化**：在 `_collaboration_handoff()` 中优化 heartbeat 播报逻辑：
   - 优先显示步骤进度：`步骤 1/4: 搜索平台基础信息（25%）`
   - 有中间结果时显示：`分析中：最新结果`
   - 降级显示时间：`仍在分析阶段（67s）`
2. **SharedContext 进度利用**：Talker 播报优先读取 `shared.thinker_progress` 中的：
   - `current_step` / `total_steps`：步骤进度
   - `partial_results`：中间结果
   - `current_stage`：当前阶段
3. **文档记录**：在 `docs/optimization-plan.md` 中新增"Talker 播报内容优化"章节。

### 回归用例（已补充）
- `test_heartbeat_broadcasts_with_step_info`：heartbeat 播报应显示步骤信息
- `test_heartbeat_broadcasts_with_partial_result`：heartbeat 播报应显示中间结果
- `test_shared_context_progress_utilization`：Talker 应利用 SharedContext 中的进度

### 后续观察点
- Thinker 步骤执行时，Talker 播报是否显示"步骤 X/Y: 步骤名称"
- Thinker 输出中间结果后，Talker 播报是否能引用最新结果
- 长任务（>2 分钟）中，播报内容是否多样化、不重复

### 后续优化建议
1. **Thinker 输出劫持**：Thinker 的阶段输出先传递给 Talker，由 Talker 重新组织后显示
2. **进度百分比可视化**：在播报中显示进度条或百分比
3. **播报内容个性化**：根据用户情绪（如催促）调整播报语气和频率
